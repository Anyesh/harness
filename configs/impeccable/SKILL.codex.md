...
1. Run `node .agents/skills/impeccable/scripts/context.mjs` once per session. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's saved design context (its PRODUCT.md and, when present, DESIGN.md, kept in `~/.impeccable/<project>/` rather than in your repo) as a markdown block, or reports `NO_CONTEXT`. Follow whatever it prints. If it reports `NO_CONTEXT`, just proceed with the task using sensible design defaults; you may offer once to run `init` to capture context there for next time, but never stop or block on it. If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Setup has already run `context.mjs`; if it reported `NO_CONTEXT` and you chose to run init, you are already in it, so finish that and skip this. Otherwise run `node .agents/skills/impeccable/scripts/context-signals.mjs` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**
...
If the first word is `craft`, setup still runs first, but [reference/craft.md](reference/craft.md) owns the rest of the flow. If you chose to run `init` first, finish it, refresh context, then resume the original command and target.
