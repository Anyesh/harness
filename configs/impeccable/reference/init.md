...
- **PRODUCT.md** (strategic): design-context file (kept in `~/.impeccable/<project>/`, not in the repo) for register, target users, product purpose, brand personality, anti-references, strategic design principles. Answers "who/what/why".
- **DESIGN.md** (visual): design-context file (kept in `~/.impeccable/<project>/`, not in the repo) for visual theme, color palette, typography, components, layout. Follows the [Google Stitch DESIGN.md format](https://stitch.withgoogle.com/docs/design-md/format/). Answers "how it looks".
...
Check what already exists. PRODUCT.md and DESIGN.md live in this project's context store; run `node .claude/skills/impeccable/scripts/context.mjs --store` to get the directory (under `~/.impeccable/<project>/`). Legacy projects may still have them at the project root, `.agents/context/`, or `docs/`. Read whichever are present with your native file tool. Also note whether `.impeccable/live/config.json` already exists (Step 6 leaves it untouched if so).
...
Write PRODUCT.md into this project's context store, never into the repo: run `node .claude/skills/impeccable/scripts/context.mjs --store` to get the directory (under `~/.impeccable/<project>/`), create it if needed, and write `PRODUCT.md` there. If a PRODUCT.md already lives there, merge into it rather than starting from scratch.
...
If you ran init by choice before another impeccable command (e.g. the user ran `/impeccable polish` and chose to set up context first), resume that original task now. Your own writes are the freshest source; no reload needed.
