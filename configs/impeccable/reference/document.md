Generate a `DESIGN.md` file in this project's context store (`~/.impeccable/<project>/`, not in the repo) that captures the current visual design system, so AI agents generating new screens stay on-brand. Run `node .claude/skills/impeccable/scripts/context.mjs --store` to get the directory.
