 ...
 *   1. ~/.impeccable/<project-slug>/ — the per-project home store, where init
 *      and document write, so the repo is never littered with PRODUCT.md
 *   2. $IMPECCABLE_CONTEXT_DIR (absolute or cwd-relative) — power-user override
 *   3. legacy in-repo cwd, .agents/context/, docs/ — read-only, so projects
 *      configured the old way keep working
 *   4. the home store dir as a "nothing found" default (init creates it there)
 *
...
export function projectStoreDir(cwd = process.cwd()) {
  // Per-project context lives under the user's home, never in the repo, so the
  // skill cannot litter a codebase with PRODUCT.md / DESIGN.md.
  const override = process.env.IMPECCABLE_HOME && process.env.IMPECCABLE_HOME.trim();
  const root = override ? override.trim() : path.join(os.homedir(), '.impeccable');
  return path.join(root, resolveProjectSlug(cwd));
}

export function resolveContextDir(cwd = process.cwd()) {
  const store = projectStoreDir(cwd);
  if (firstExisting(store, [...PRODUCT_NAMES, ...DESIGN_NAMES])) {
    return store;
  }
  ...
  // Legacy in-repo locations stay readable so older projects keep working, but
  ...
  // Nothing found: return the home store because that is where init creates it.
  return store;
}
...
async function cli() {
  if (process.argv.includes('--store')) {
    process.stdout.write(projectStoreDir(process.cwd()) + '\n');
    return;
  }
  ...
  if (!ctx.hasProduct) {
    const store = projectStoreDir(process.cwd());
    const parts = [
      `NO_CONTEXT: No saved design context for this project yet (looked in ` +
      `${store}). Proceed with the current task using sensible design defaults. ` +
      `You MAY offer once to run \`init\` to capture brand and product context ` +
      `there for future sessions, but do not stop, block, or require it.`,
    ];
