#!/usr/bin/env node
// humanize — shared configuration resolver
//
// Resolution order for default mode:
//   1. HUMANIZE_DEFAULT_MODE environment variable
//   2. Config file: ~/.config/humanize/config.json
//   3. 'on'
//
// Platform detection: checks __dirname path for .cursor vs .claude
// to determine output format. Shared between Claude Code and Cursor.
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'humanize');
  }
  ...
  return path.join(os.homedir(), '.config', 'humanize');
}
...
function getDefaultMode() {
  const envMode = process.env.HUMANIZE_DEFAULT_MODE;
...
function getFlagPath() {
  ...
  return path.join(configDir, '.humanize-active');
}
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(realFlagDir, `.humanize-active.${process.pid}.${Date.now()}`);
