...
codex_deploy_config() {
  ...
  if [[ -f "$config_src" ]] && ! head -1 "$config_src" | grep -q '^# harness does not manage'; then
    deploy_rendered_template "$config_src" "$CODEX_CONFIG_DIR/config.toml" "configs/codex/config.toml.tmpl" "codex config.toml" || true
  fi

  if [[ -f "$instructions_src" ]]; then
    deploy_rendered_template "$instructions_src" "$CODEX_CONFIG_DIR/instructions.md" \
      "configs/codex/instructions.md.tmpl" "codex instructions.md" || true
  fi

  if [[ -f "$hooks_src" ]]; then
    deploy_rendered_template "$hooks_src" "$CODEX_CONFIG_DIR/hooks.json" "configs/codex/hooks.json" "codex hooks.json" || true
  fi
